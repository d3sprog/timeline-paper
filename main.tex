...
\lstdefinelanguage{timeline}{
    morekeywords={IF, AND, STEP, COS, RADIANS, MAX, SIN, POWER, SUM, POINT, GRAPH, STROKECOLOR, LINE, OVERLAY, AXES, KEYPRESS, IMAGE, FALSE, NOT, MIN,FILLCOLOR, COLUMN, APPEND, COORD, ASPECTRATIO, INDEX, INDIRECT, OFFSET},
...
\qquad \omega_{k+1}(a)(1+t) = \omega_k(t)
\end{array}
\]
...
\autoref{fig:coefftypes}. We omit rules for other kinds of expressions as well as rules for accessing
a cell range. All of those remain the same as before, except that they pass the unmodified context
$\Omega\coeff t$ to their premises.
...
\vspace{-0.25em}
\raggedright
$\boxed{\textnormal{Coeffect-based checking:}~~\Omega\coeff t \vdash F~\ident{ok} }$

\[
\inferrule
  {\Omega(a)\ge t_0\!+\!t \quad t = \ident{max}(t_1,t_2)}
  {\Omega \coeff t_0 \vdash a[-t_1\!:\!-t_2]~\ident{ok}}
\qquad
\inferrule
  {\Omega(a) = \infty}
...
\]
\vspace{-0.5em}
\caption{Coeffect-based system for checking past value accesses}
...
\[
\inferrule
  {\sigma, \omega(a)(t)=V}{\sigma, \omega \vdash a[-t] \,!`\, V}
\qquad
\inferrule
  {a\notin\ident{dom}(\omega) \quad \sigma(a)=F \quad \sigma,\textsf{shift}_t(\omega) \vdash F \Downarrow V}
...
\begin{figure}[t]
\raggedright
$\boxed{\textnormal{Coeffect-based evaluation:}~~\sigma \Downarrow_n \gamma}$
...
\\[1em]
\sigma \Downarrow_n \gamma_n \iff \sigma, \omega'_{n-1} \Downarrow \gamma_n~
  \textnormal{where}~\forall k \in 0..n\!-\!2\!:\\[0.25em]
~
\qquad \omega_{k+1}(a)(1) = \gamma_{k+1}(a)\quad
  (\textnormal{where}~\sigma, \omega'_k \Downarrow \gamma_{k+1})\\[0.25em]
\qquad \omega_{k+1}(a)(1+t) = \omega'_k(t)\\[0.25em]
