...
also be easily encoded in an ordinary two-dimensional spreadsheet (using rows for time steps).
...
experiences prototyped in Cuscus \cite{marasoiu-2019-cuscus}. Like in Cuscus, the graphical shape of a
...
The problem of type inference and type checking for spreadsheets has been addressed by others
...
with computer science background. Although the small scale of the study limits generalisability,
...
\end{enumerate}
%
Three of the participants completed all four parts. Our key findings were:
%
\begin{enumerate}[label=\alph*),leftmargin=1.5em]
\item An expert spreadsheet user quoted earlier (\autoref{sec:walkthrough}) attempted to use an analytical
...
\subsection{Using the two dimensions}
\label{sec:case-studies-using}

The formative study (\autoref{sec:user-study}) identified the difficulty with embedding
three-dimensional data into two-dimensional grid as one of the challenges when using conventional
spreadsheets. We can now revisit the four case studies, consider how they use the grid
and whether (and how) they could be embedded in ordinary two-dimensional spreadsheet:
%
...
\item Planetary orbit (\caseref{sec:case-physics}{1}) and cannonball (\autoref{sec:walkthrough})
  ...
  the problem discussed earlier (\autoref{fig:tables}). With dimensions
  $4\times 18$ and $5\times 8$, this would involve a very wide table or a large number of auxiliary tables.
...
\item Flocking (\caseref{sec:case-abm}{2}) uses both an $\textit{entities}\times\textit{attributes}$
  ...
  tables for pairwise comparison to calculate distances and other interactions. Doing so in an
  ordinary spreadsheet would require a wide table and dynamic index calculation.
...
\item Moving average crossover (\caseref{sec:case-finance}{3}) does not use two-dimensional tables
  ...
  live animated chart and could support streaming data. This can also be achieved in two-dimensions \cite{chang-2015-streaming}.
...
it would require using large amount of space, that is a situation encountered in part 3 of
our formative study that two of the participants judged as \emph{miserable} and
\emph{painful}, respectively.
...
spreadsheets achieve their usability does not have an established definitive answer.
...
using the cognitive dimensions of notations framework \cite{green-1996-cogdims}. This has
identified several notable characteristics. Spreadsheets can support \emph{closeness of mapping}
where the program domain is closer to the problem domain if the problem fits naturally with a
two-dimensional grid. They do not suffer from high \emph{viscosity}, meaning that changes are
...
well with a broader range of application domains. In addition to tabular data analysis, these
...
edit-triggered recomputation, automatically recomputing the spreadsheet from the first time-step if
necessary.
...
\Timeline also has a higher \emph{viscosity} than conventional spreadsheets when the user wants
to modify a value at certain time steps. When encoding time as rows, this can be done by selecting
the affected range and modifying the value. When time exists in a single time-dependent formula,
such change can only be done by using \tml{IF} function with a condition based on the current
\tml{STEP()}.
Modifying the logic of a single entity is, however, easier in \Timeline where the logic for all
time steps exists in a single formula, as opposed to multiple copies of the same formula.
...
in which individuals are born and die can be only modelled by pre-allocating a fixed table of
entities. Moreover, all cells advance on a single global clock and \Timeline has no analogue of the
...
as finance where users often work with data indexed by time. However, generalising the notion of time
...
covers work on coeffect systems, data visualization and typed holes. This section
provides an overview of the remaining main related research areas.
...
The \Timeline execution draws inspiration from synchronous dataflow languages such as
Lucid, Lustre and Esterel~\cite{benveniste-2003-sync,ashcroft-1977-lucid,caspi-1987-lustre,halbwachs-1991-lustre,berry-1992-esterel}.
Our programming model is most closely related to Lustre, whose clock calculus~\cite{caspi-1992-clocks}
is a predecessor of our checking of past accesses. Functional reactive programming systems
\cite{elliott-1997-fran,wan-2000-frp,courtney-2003-yampa} do not use discrete time, but
event-based systems are more similar to our approach \cite{wan-2002-efrp}.
Space leak problems have been a challenge for the implementation of FRP
systems~\cite{krishnaswami-2013-leaks,bahr-2022-frp}, not shared by our system.
...
The key observation in this paper is that the design and implementation of a spreadsheet system
...
guarantee memory-bounded execution using coeffects, the formula editor can leverage the theory of
typed holes and we can support rich charts by using ideas rooted in grammar of graphics.
There is more to spreadsheets than meets the (programming language researcher's) eye!
